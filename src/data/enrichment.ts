...
export const BOOK_ENRICHMENT: Record<string, BookEnrichment> = {
  ...
  "piranesi": {
    ...
    faq: [
      { q: { en: "What does the ending of Piranesi mean?", zh: "《皮拉內西》的結局意味著什麼？" }, a: { en: "It means acceptance rather than restoration. The narrator is not returned to the person he was before the House, and the novel treats the self that formed inside captivity as a real self rather than a damaged copy. See the section 'What does the Piranesi ending mean?' on this page for the spoiler-safe explanation.", zh: "它指向「接受」而非「復原」。主角並未被還原成進入 The House 之前的那個人，而小說把在囚禁中形成的自我視為真正的自我，而非受損的副本。本頁上方段落有無雷版說明。" } },
      { q: { en: "Is Piranesi's ending open or ambiguous?", zh: "《皮拉內西》的結局是開放式還是曖昧？" }, a: { en: "Both, by our classification. The immediate danger is resolved, so it is not an unfinished plot; what stays open is meaning — whether the narrator belongs to the House or the world, and which self he continues as. We tag it Ambiguous for that reason.", zh: "依本站分類兩者兼有。眼前的危險已經解決，因此不是劇情未完；真正開放的是意義——他究竟屬於 The House 還是現實世界、以哪個自我繼續活下去。因此標記為 Ambiguous。" } },
