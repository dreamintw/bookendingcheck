...
const FAQ_EN = [
  ...
  { q: "Will you ever expand the genre detail pages?", a: "Yes — once a genre has enough fully written-up books to support an intro, FAQ, and ending profile, that genre detail page will be added back. For now we only keep this overview indexed." },
...
const FAQ_ZH = [
  ...
  { q: "那我應該用什麼入口？", a: "想按結局挑書，請看 HE / BE / OE / 苦甜 等 collection。想避開特定內容，請看避雷標籤總覽。只記得書名，請用全書庫搜尋。" },
  { q: "genre detail 頁未來會補上嗎？", a: "會。等某個類型下已經深度整理的書夠多、可以撐起導讀、FAQ 與結局輪廓時，該類型的 detail 頁會重新開放索引。目前只保留這個總覽頁。" },
  { q: "你們有沒有覆蓋言情、奇幻、推理、文學小說？", a: "有，覆蓋在我們深度整理過的書裡。實用的入口是下方的 collection，它們是跨類型按閱讀意圖切的。" },
    ...
    <main className="mx-auto max-w-3xl w-full px-4 py-12 flex-1">
      ...
      {lang === "zh" ? (
        <div className="prose prose-sm dark:prose-invert max-w-none mb-8 text-foreground/90 leading-relaxed">
          <p>類型只是粗篩。NovelCheck 不打算和現有的「2024 最佳奇幻」「TOP 50 推理」這種網站搶 SEO，因為這些清單通常沒回答讀者最在意的問題：「結局好不好？」「會不會出現我最怕的橋段？」「能不能先看一頁就決定要不要繼續？」我們做的是把這層讀前決策疊在類型之上。</p>
          <p>因此本站不會用 genre 當主要入口，而是用 collection、avoidance（避雷）、ending 結局類型來組織書庫。在 AdSense 審核期間，個別 genre detail 頁暫時不索引，等該類型下深度整理的書足以撐起導讀與 FAQ，再重新開放。</p>
        ...
        <div className="prose prose-sm dark:prose-invert max-w-none mb-8 text-foreground/90 leading-relaxed">
          ...
          <p>That's why genre isn't our primary entry. We organize the catalog around collections, avoidance (trigger warnings), and ending type instead. During AdSense review, individual genre detail pages are temporarily not indexed; once a given genre has enough fully written-up books to support its own intro and FAQ, its detail page will be brought back.</p>
